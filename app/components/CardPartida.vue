<template>
  ...
  >
    ...
    <div class="flex gap-4 items-center justify-center">
      ...
      <UTooltip
        v-else
        :text="simulacao.get(partida.id) ? simulacao.get(partida.id).gols_mandante : undefined"
      >
      ...
      <UTooltip
        v-else
        :text="simulacao.get(partida.id) ? simulacao.get(partida.id).gols_visitante : undefined"
      >
...
const data = computed(() => {
  return props.partida.data
    ? format(new Date(props.partida.data.split('-')), 'd MMM', { locale: ptBR })
    : 'A definir'
})
