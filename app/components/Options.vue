    ...
    <UPopover :mode="popoverMode">
      ...
      <template #content>
        <div class="flex flex-col">
          <span
            @click="atribuirVitoriaSimplesMandante"
          ...
          >
          <span
            @click="atribuirVitoriaSimplesVisitante"
          ...
          <USeparator
            v-if="partida.status != 'finalizada' && simulacao.get(partida.id)"
            class="py-1"
          />
          <span
            @click="confirm = true"
            v-if="partida.status != 'finalizada' && !confirm && simulacao.get(partida.id)"
          ...
          <span
            @click="limparSimulacao(partida.id)"
            v-if="partida.status != 'finalizada' && confirm"
