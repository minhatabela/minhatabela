...
export const useSimulador = () => {
  ...
  watch(status, async value => {
    if (value === 'success' && useSupabaseUser().value) {
      ...
      const sims = diff.map(m => {
        return Object.assign(simulacao.value.get(m), { id: crypto.randomUUID() })
      })
  ...
  const jogosRodada = computed(() => {
    return useOrderBy(filtraJogosRodada(partidas.value || [], rodada_navegavel.value), o =>
      isDefined(o.data) && isDefined(o.hora)
        ? new Date(o?.data || 0).setHours(...o?.hora?.split(':'))
        : 0
    )
  })
  ...
  async function salvarSimulacao(simulada: Tables<'simulacao'>) {
    ...
    if (user.value) {
      ...
      const { error, data } = await client
        .from('simulacao')
        .upsert(simulada, { returning: 'minimal' })
        .select('id')
