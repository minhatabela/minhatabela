<template>
  <UTabs v-model="view" :content="false" :items="items" class="w-full mb-4" />
  <UCard v-if="view === MatchesView.STORED" variant="subtle">
    <UTable :loading="status !== 'success'" :data="data?.partidas" :columns="colunas">
      <template #mandante_nome_popular-cell="{ getValue, row }">
        <TeamBadge :name="getValue()" :emblem="row.original.mandante.escudo" />
      </template>
      <template #visitante_nome_popular-cell="{ getValue, row }">
        <div class="flex justify-end">
          <TeamBadge :name="getValue()" :emblem="row.original.visitante.escudo" />
        </div>
      </template>
      <template #status-cell="{ getValue, row }">
        <UBadge class="rounded-full" variant="subtle" :icon="statusPartida[getValue()].icon"
          :color="statusPartida[getValue()].color" size="lg">{{
            statusPartida[getValue()].label }}</UBadge>
      </template>
      <template #inconsistencias-cell="{ getValue, row }">
        <UButton @click="setPickedPartida(row.original)" class="rounded-full" variant="ghost"
          :icon="Object.values(getValue()).length ? 'i-lucide-circle-alert' : 'i-lucide-check-circle'"
  ...
  <UCard v-else>
    <UTable :loading="status !== 'success'" :data="data?.partidasParaCriar" :columns="colunasNovasPartidas">
      <template #mandante_nome_popular-cell="{ getValue, row }">
        <TeamBadge :name="getValue()" :emblem="row.original.mandante.escudo" />
      </template>
      <template #visitante_nome_popular-cell="{ getValue, row }">
      ...
      <template #actions-cell="{ row }">
        <UButton color="neutral" variant="ghost" :loading="loadingCreate.has(row.original.numero)" :icon="'i-lucide-plus'" @click="storeMatch(row.original)" />
      </template>
    </UTable>
  </UCard>
